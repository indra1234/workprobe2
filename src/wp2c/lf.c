...
void main(void)
{
	...
	#endif
	LCD_Init();
	LCD_String(0x80,"Line Follower.");	
	while(1)
	{
		if(S1==0 && S2==0)
		{
              LCD_String(0xC0,"Forward "); DCM_Forward();
		}
		else if(S1==1 && S2==0)
		{
			LCD_String(0xC0,"Right   "); DCM_Right();
		}
		else if(S1==0 && S2==1)
		{
			LCD_String(0xC0,"Left    "); DCM_Left();
		}
		else 
		{
			LCD_String(0xC0,"Stop    "); DCM_Stop();
		}
	}
